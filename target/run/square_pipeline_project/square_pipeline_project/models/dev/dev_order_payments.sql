  ...
  create or replace view `we-analysis`.`Sales_Data_dev`.`dev_order_payments`
  OPTIONS()
  as 
...
WITH payments AS (
    ...
    FROM `we-analysis`.`Sales_Data_staging`.`stg_payments` p
    JOIN `we-analysis`.`Sales_Data_staging`.`stg_orders` o
